...
module.exports = {
    ...
    makeComment: (postId, data) => {
        let comments = JSON.parse(fs.readFileSync('/home/app/community_api/api/db/comment.json', 'utf8'));
        try {
            data.postId = postId;
            ...
            comments.push(data);
            fs.writeFileSync('/home/app/community_api/api/db/comment.json',JSON.stringify(comments),'utf8');
    ...
    readComment: (commentId) => {
        const comment = db.find(comment => comment.id == commentId);
    ...
    modifyComment: (commentId, data) => {
        let commentsAll = JSON.parse(fs.readFileSync('/home/app/community_api/api/db/comment.json', 'utf8'));
        ...
        if(comments !== undefined && !comments.deleted_at){
            try{
                ...
                fs.writeFileSync('/home/app/community_api/api/db/comment.json', JSON.stringify(commentsAll), 'utf8');
                return 1;
            }
            catch(err) {
                console.log(err)
